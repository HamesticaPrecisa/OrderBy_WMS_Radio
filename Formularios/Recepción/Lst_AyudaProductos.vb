...
Public Class Lst_AyudaProductos
    ...
    Private Sub listar()
        ...
        If buscar <> "" Then
            sqlListar = " SELECT mae_codi, mae_descr , mae_codcli as CODCLI " & _
                        " FROM maeprod WHERE (mae_codi LIKE '%" + buscar + "%' OR mae_descr LIKE '%" + buscar + "%') AND " + filtro
        Else
            sqlListar = " SELECT mae_codi, mae_descr , mae_codcli as CODCLI " & _
                        " FROM maeprod WHERE " + filtro
        End If
